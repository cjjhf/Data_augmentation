...
def adjust_brightness(image, brightness, top_left, bottom_right):
    roi = image[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]

    # 创建与原始图片相同大小的零矩阵
    blank = np.zeros(roi.shape, roi.dtype)

    # 调整图片亮度
    adjusted_image = cv2.addWeighted(roi, 1 + brightness / 100, blank, 0, brightness)

    image[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]] = adjusted_image
...
def move_bounding_boxes(xml_file, image_folder, output_file):
    ...
    for i in range(1, number):
        ...
        for obj in root.iter('object'):
            ...
            bit_img[:, :, 0] = adjust_brightness(bit_img[:, :, 0], brightness, top_left,
                                                 bottom_right)
            bit_img[:, :, 1] = adjust_brightness(bit_img[:, :, 1], brightness, top_left,
                                                 bottom_right)
            bit_img[:, :, 2] = adjust_brightness(bit_img[:, :, 2], brightness, top_left,
                                                 bottom_right)
...
output_folder = r'E:\process_dataset\kitti\kitti_brightness\JPEGImages'  # 输出文件夹路径
