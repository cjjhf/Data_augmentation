...
def move_bounding_boxes(xml_file, image_folder, output_file):
    ...
    for i in range(1, number):
        ...
        for obj in root.iter('object'):
            ...
            image_noise = np.random.poisson(lam=2, size=((ymax - ymin), (xmax - xmin))).astype(dtype='uint8')

            bit_img[ymin:ymax, xmin:xmax, 0] = image_noise
            bit_img[ymin:ymax, xmin:xmax, 1] = image_noise
            bit_img[ymin:ymax, xmin:xmax, 2] = image_noise
...
output_folder = r'E:\process_dataset\kitti\kitti_noise\JPEGImages'  # 输出文件夹路径
